...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  const gamify=document.getElementById('gamify');
  const yearEl=document.getElementById('year'); if(yearEl) yearEl.textContent=new Date().getFullYear();
  if(missed) missed.addEventListener('input',()=> missedVal.textContent=missed.value);
  ...
  if(form) form.addEventListener('change', ()=>{
    ...
    const total=8;
    const answered=Array.from(new FormData(form).entries()).filter(([k,v])=>/^q[1-8]$/.test(k)&&v!=='').length;
    if(progress) progress.style.width=Math.round((answered/total)*100)+'%';
  });
  ...
  if(resetBtn) resetBtn.addEventListener('click', ()=>{ if(form) form.reset(); if(missedVal) missedVal.textContent='0'; if(progress) progress.style.width='0%'; if(result) result.classList.add('hidden'); });
});
